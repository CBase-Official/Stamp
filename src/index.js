...
async function initContract() {
    ...
    console.log("dddd--",window.nearConfig.contractName)
    window.contract = await new nearlib.Contract(acct, window.nearConfig.contractName, {
        // View methods are read only. They don't modify the state, but usually return some value.
        viewMethods: ['get_token_owner','stamp_info','total','count_id_token'],
        // Change methods can modify the state. But you don't receive the returned value when called.
        changeMethods: ['check_access','mint_token','transfer'],
...
window.nearInitPromise = initContract().then(() => {
  ReactDOM.render(<App contract={window.contract} near={window.near} accountId={window.accountId} wallet={window.walletAccount} />,
